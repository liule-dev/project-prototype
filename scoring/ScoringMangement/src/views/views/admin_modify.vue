...
// 初始化获取年级和学科数据
onMounted(() => {
  api.fetchData('http://localhost:8000/grades/', grades, '获取年份数据失败');
  api.fetchData('http://localhost:8000/subjects/', subjects, '获取等级数据失败');
});

// 从父组件接收数据并回显
const admin_openDialog = async (rowData) => {
...
<template>
  ...
  >
    ...
    >
      ...
      <!-- 等级（可修改） -->
      <el-form-item label="等级" prop="subject">
        <el-select
      ...
      <!-- 状态（不可修改） -->
      <el-form-item label="状态" prop="question_type">
        <el-select
