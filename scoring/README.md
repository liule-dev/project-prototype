...
### 1. AI智能批改 (ReAct Agent)
- **Agent 架构**：基于 ReAct (Reasoning + Acting) 思维模式，引导模型先进行“关键点推理”再执行“分数决策”。
- **Prompt 工程**：标准化评分维度并调优 `temperature=0.2`，结合 JSON 结构化输出解析器。
- **三层防护**：格式约束 + 正则提取 + 业务规则校验（范围限制），解决 LLM 输出不可控难题。
- **性能指标**：单卷批改耗时 < 5秒，阅卷准确率达 89%。
