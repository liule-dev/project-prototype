...
- ⚡ **高性能**: 三级缓存架构，缓存命中场景可显著降低数据库访问
- 🚀 **高并发**: 支持通过 Celery Worker、连接池和缓存进行水平扩展，实际容量需以压测结果为准
- 🎯 **智能化**: 多智能体协作，自动分诊至专科医生
- 📊 **可扩展**: Celery 异步任务队列，便于按队列拆分和扩容
- 🔒 **工程化**: 提供认证、日志和监控接入能力
...
用户请求
  ↓
┌─────────────────────┐
│ L1: 内存缓存         │ ← 进程内 LRU，低延迟访问
└─────────────────────┘
  ↓ 未命中
┌─────────────────────┐
│ L2: Redis 缓存       │ ← TTL + 抖动防雪崩
└─────────────────────┘
  ↓ 未命中
┌─────────────────────┐
│ L3: MySQL 数据库     │ ← 连接池访问，延迟以实测为准
└─────────────────────┘
```
...
**性能说明**：
- 热点问答命中 L1/L2 缓存后可减少数据库查询和重复推理开销
- 缓存命中率需结合 L1 内存缓存、L2 Redis 缓存和实际业务访问分布统计
- 数据库负载下降幅度需通过线上监控或压测报告验证
...
| 优化项 | 说明 | 验证方式 |
|--------|------|----------|
| **Redis 连接池** | 提高缓存访问并发能力 | 观察 Redis 连接池等待时间与错误率 |
| **MySQL 连接池** | 降低高并发下连接创建开销 | 观察数据库连接数、慢查询和超时 |
| **Celery 并发** | 将 AI、文件、数据任务拆分到不同队列 | 观察队列积压、任务耗时和失败率 |
| **LLM 流式输出** | WebSocket 直接使用模型流式响应，降低首字等待时间 | 统计首字响应时间和完整响应时间 |
| **缓存策略** | L1/L2 缓存减少重复查询 | 统计 L1 命中率、L2 命中率和回源次数 |
| **混合检索与重排序** | 提升召回质量，但会增加重排序耗时 | 分别统计检索耗时、重排序耗时和总耗时 |

### 性能测试

运行性能验证脚本，并补充 Locust 或 Apache Bench 压测结果：
...
建议记录以下指标：
- 首字响应时间、完整响应时间、P95/P99 延迟
- L1/L2 缓存命中率、回源次数
- Celery 队列积压、任务失败率
- CPU、内存、GPU、Redis、MySQL 资源使用率
